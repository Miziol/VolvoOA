...
protected:
    ...
    void scenario_ValidProtocolVersion(uint16_t protocolVersion) {
        ...
        EXPECT_CALL(usbWrapperMock_, fillControlSetup(NotNull(), LIBUSB_ENDPOINT_IN | USB_TYPE_VENDOR,
                                                      ACC_REQ_GET_PROTOCOL, 0, 0, sizeof(protocolVersion)));

        AccessoryModeProtocolVersionQuery::Pointer query(
            std::make_shared<AccessoryModeProtocolVersionQuery>(ioService_, usbWrapperMock_, usbEndpointMock_));
...
BOOST_FIXTURE_TEST_CASE(AccessoryModeProtocolVersionQuery_InvalidProtocolVersion,
                        AccessoryModeProtocolVersionQueryUnitTest) {
    ...
    EXPECT_CALL(*usbEndpointMock_, controlTransfer(_, _, _))
        .WillOnce(DoAll(SaveArg<0>(&buffer), SaveArg<2>(&usbEndpointPromise)));
    EXPECT_CALL(usbWrapperMock_, fillControlSetup(NotNull(), LIBUSB_ENDPOINT_IN | USB_TYPE_VENDOR, ACC_REQ_GET_PROTOCOL,
                                                  0, 0, sizeof(protocolVersion)));
...
BOOST_FIXTURE_TEST_CASE(AccessoryModeProtocolVersionQuery_RejectWhenInProgress,
    ...
    secondPromise->then(
        std::bind(&AccessoryModeQueryPromiseHandlerMock::onResolve, &secondPromiseHandlerMock, std::placeholders::_1),
