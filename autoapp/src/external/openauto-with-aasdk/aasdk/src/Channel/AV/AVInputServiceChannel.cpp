...
void AVInputServiceChannel::receive(IAVInputServiceChannelEventHandler::Pointer eventHandler) {
    ...
    receivePromise->then(
        std::bind(&AVInputServiceChannel::messageHandler, this->shared_from_this(),
                  std::placeholders::_1,
                  eventHandler),
        std::bind(&IAVInputServiceChannelEventHandler::onChannelError, eventHandler,
                  std::placeholders::_1));
...
void AVInputServiceChannel::sendAVInputOpenResponse(
    ...
    auto message(std::make_shared<messenger::Message>(channelId_, messenger::EncryptionType::ENCRYPTED,
                                                      messenger::MessageType::SPECIFIC));
    message->insertPayload(
        messenger::MessageId(proto::ids::AVChannelMessage::AV_INPUT_OPEN_RESPONSE).getData());
    message->insertPayload(response);
...
void AVInputServiceChannel::sendAVMediaWithTimestampIndication(
    ...
    auto message(std::make_shared<messenger::Message>(channelId_, messenger::EncryptionType::ENCRYPTED,
                                                      messenger::MessageType::SPECIFIC));
    message->insertPayload(
        messenger::MessageId(proto::ids::AVChannelMessage::AV_MEDIA_WITH_TIMESTAMP_INDICATION).
        getData());
...
void AVInputServiceChannel::handleAVChannelSetupRequest(const common::DataConstBuffer &payload,
                                                        IAVInputServiceChannelEventHandler::Pointer
                                                        eventHandler) {
