...
AndroidAutoEntity::~AndroidAutoEntity() {
    qDebug() << "[AndroidAutoEntity] destroy.";
}
...
void AndroidAutoEntity::onVersionResponse(uint16_t majorCode,
    ...
    qInfo() << "[AndroidAutoEntity] version response, version: " << majorCode << "." << minorCode
            << ", status: " << status;

    if (status == aasdk::proto::enums::VersionResponseStatus::MISMATCH) {
        qCritical() << "[AndroidAutoEntity] version mismatch.";
        this->triggerQuit();
    } else {
        qInfo() << "[AndroidAutoEntity] Begin handshake.";
        ...
        try {
            ...
            controlServiceChannel_->receive(this->shared_from_this());
        } catch (const aasdk::error::Error &e) { this->onChannelError(e); }
    }
}

void AndroidAutoEntity::onHandshake(const aasdk::common::DataConstBuffer &payload) {
    qInfo() << "[AndroidAutoEntity] Handshake, size: " << payload.size;

    try {
        cryptor_->writeHandshakeBuffer(payload);

        if (!cryptor_->doHandshake()) {
            qInfo() << "[AndroidAutoEntity] continue handshake.";
            ...
            qInfo() << "[AndroidAutoEntity] Auth completed.";
...
void AndroidAutoEntity::onServiceDiscoveryRequest(const aasdk::proto::messages::ServiceDiscoveryRequest &request) {
    qInfo() << "[AndroidAutoEntity] Discovery request, device name: " << QString::fromStdString(request.device_name())
            << ", brand: " << QString::fromStdString(request.device_brand());
    ...
    serviceDiscoveryResponse.set_left_hand_drive_vehicle(configuration_.isLeftHandDrive());
    ...
    serviceDiscoveryResponse.set_hide_clock(!configuration_.getShowClock());
...
void AndroidAutoEntity::onAudioFocusRequest(const aasdk::proto::messages::AudioFocusRequest &request) {
    qInfo() << "[AndroidAutoEntity] requested audio focus, type: " << request.audio_focus_type();
    ...
    qInfo() << "[AndroidAutoEntity] audio focus state: " << audioFocusState;
...
void AndroidAutoEntity::onShutdownRequest(const aasdk::proto::messages::ShutdownRequest &request) {
    qInfo() << "[AndroidAutoEntity] Shutdown request, reason: " << request.reason();
...
void AndroidAutoEntity::onShutdownResponse(const aasdk::proto::messages::ShutdownResponse &) {
    qInfo() << "[AndroidAutoEntity] Shutdown response ";
    this->triggerQuit();
}

void AndroidAutoEntity::onNavigationFocusRequest(const aasdk::proto::messages::NavigationFocusRequest &request) {
    qInfo() << "[AndroidAutoEntity] navigation focus request, type: " << request.type();
...
void AndroidAutoEntity::onChannelError(const aasdk::error::Error &e) {
    qCritical() << "[AndroidAutoEntity] channel error: " << e.what();
    this->triggerQuit();
}
    ...
    promise->then(
        ...
        [this, self = this->shared_from_this()](auto error) {
            if (error != aasdk::error::ErrorCode::OPERATION_ABORTED &&
                ...
                qCritical() << "[AndroidAutoEntity] ping timer exceeded.";
                this->triggerQuit();
            }
        });
